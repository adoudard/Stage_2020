...
ksi=np.array([B12[0],B12[1],B34[0],B34[1],B56[0],B56[1],B78[0],B78[1],B910[0],B910[1],A11[0],A12[0],A11[1],A12[1],A11[2],A12[2],A11[3],A12[3]])
ksip=np.array([item for pair in zip(A1, A2) for item in pair]+[item for pair in zip(A3, A4) for item in pair]+[item for pair in zip(A5, A6) for item in pair]+[item for pair in zip(A7, A8) for item in pair]+[item for pair in zip(A9, A10) for item in pair]+[item for pair in zip(A11, A12) for item in pair][:-2])
...
plt.subplot(236)
ray=np.sqrt(np.max((x180e-XFG180E)**2+(y180e-YFG180E)**2))
...
#1.1 Isocentre bras avec C=0,T=0

u=np.array([A1[0],A3[4],A5[0],A7[4]])
v=np.array([A1[0],A4[4],A6[0],A8[4]])
...
PG0=0.5*np.array([np.max(xg)-np.min(xg), np.max(yg)-np.min(yg), np.max(zg)-np.min(zg)])

#1.2 Isocentre bras avec C=72,T=0

u=np.array([A1[1],A3[3],A5[1],A7[3]])
v=np.array([A1[1],A4[3],A6[1],A8[3]])

xg=u
yg=v*np.cos(theta)
zg=v*np.sin(theta)

PG72=0.5*np.array([np.max(xg)-np.min(xg), np.max(yg)-np.min(yg), np.max(zg)-np.min(zg)])

#1.3 Isocentre bras avec C=144,T=0

u=np.array([A1[2],A3[2],A5[2],A7[2]])
v=np.array([A1[2],A4[2],A6[2],A8[2]])

xg=u
yg=v*np.cos(theta)
zg=v*np.sin(theta)

PG144=0.5*np.array([np.max(xg)-np.min(xg), np.max(yg)-np.min(yg), np.max(zg)-np.min(zg)])

#1.4 Isocentre bras avec C=216,T=0

u=np.array([A1[3],A3[1],A5[3],A7[1]])
v=np.array([A1[3],A4[1],A6[3],A8[1]])

xg=u
yg=v*np.cos(theta)
zg=v*np.sin(theta)

PG216=0.5*np.array([np.max(xg)-np.min(xg), np.max(yg)-np.min(yg), np.max(zg)-np.min(zg)])

#1.5 Isocentre bras avec C=180,T=0

u=np.array([A1[4],A3[0],A5[4],A7[0]])
v=np.array([A1[4],A4[0],A6[4],A8[0]])

xg=u
yg=v*np.cos(theta)
zg=v*np.sin(theta)

PG288=0.5*np.array([np.max(xg)-np.min(xg), np.max(yg)-np.min(yg), np.max(zg)-np.min(zg)])

#2. Isocentre bras moyen

Pm=(PG0+PG72+PG144+PG216+PG288)/5
